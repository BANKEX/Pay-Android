...
* Импортировать проект в `Android Studio` и отредактировать следующие данные:
    ...
    * `package name`, - необходимо корректно изменить имя пакета до загрузки в хоккей, предварительно согласовав с Руководителем/Заказчиком. Для смены в проекте создаем новый package. Перетаскиваем туда все необходимое из старого package. Делаем поиск по файлам с заменой старого package на новый. Удаляем старый package.
...
* Добавить необходимые файлы в проект (исключая iml, builds, и тд.) с комментарием `initial commit`
* Запушить изменения на сервер.

## Step 2: CI Configuration

Сборки:
* Автоматическая сборка и проверка кода с помощью detekt, ktlint, lint при push евентах
* `release` сборки при тегах вида `v1.0.0r`
* `releaseQa` сборки при тегах вида `v1.0.0t`

### Jenkins
1. В `Gitlab` включить `Deploy key` `jenkins` в настройках проекта.
2. В `Gitlab` добавить пользователя Jenkins в проект.
3. Перейти в `Jenkins`. Переидти в папку со сборками `template` проекта. Создать копию этой папки через Copy Folder.
4. Настроить сборки чтобы они смотрели на корректный репозиторий в Gitlab. Добавить дополнительные шаги сборки при необходимости.
5. Добавить в `Gitlab` `Web hook` с `URL` из конфигурации билдов дженкинса. Для примера можно посмотреть проект шаблона.
...
* Добавить приложение в крашлитикс собрав локально `releaseQa` и запустив эту сборку на эмуляторе или телефоне. Для сборки `releaseQa` можно использовать `signingConfig signingConfigs.debug` и ключи фабрика можно взять из `debug` buildType.

Переидти в фабрик и убедиться что приложение было добавлено в крашлитикс в организацию E-Legion.
Собрать `releaseQa` в бету крашлитикса на CI пушнув тег и убедиться в том, что она была туда загружена.
