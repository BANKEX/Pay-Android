...
class TabParser {
    ...
    @NonNull
    private final BottomBarTab.Config mDefaultTabConfig;

    @NonNull
    private final XmlResourceParser mParser;

    @Nullable
    private List<BottomBarTab> mTabs = null;

    TabParser(@NonNull Context context, @NonNull BottomBarTab.Config defaultTabConfig, @XmlRes int tabsXmlResId) {
        this.mContext = context;
        this.mDefaultTabConfig = defaultTabConfig;
        this.mParser = context.getResources().getXml(tabsXmlResId);
    }
    ...
    public List<BottomBarTab> parseTabs() {
        if (mTabs == null) {
            mTabs = new ArrayList<>(AVG_NUMBER_OF_TABS);
            try {
                int eventType;
                do {
                    eventType = mParser.next();
                    if (eventType == XmlResourceParser.START_TAG && TAB_TAG.equals(mParser.getName())) {
                        BottomBarTab bottomBarTab = parseNewTab(mParser, mTabs.size());
                        mTabs.add(bottomBarTab);
                    }
    ...
    private BottomBarTab tabWithDefaults() {
        ...
        tab.setConfig(mDefaultTabConfig);
    ...
    private String getTitleValue(@NonNull XmlResourceParser parser, @IntRange(from = 0) int attrIndex) {
        ...
        return titleResource == RESOURCE_NOT_FOUND
                ? parser.getAttributeValue(attrIndex) : mContext.getString(titleResource);
    }
    ...
    @StringDef({ID, ICON, TITLE, INACTIVE_COLOR, ACTIVE_COLOR, BAR_COLOR_WHEN_SELECTED,
            BADGE_BACKGROUND_COLOR, BADGE_HIDES_WHEN_ACTIVE, IS_TITLELESS})
